...
if (!class_exists('EHX_Donate_Event_Shortcode')) {

    class EHX_Donate_Event_Shortcode
    {
        ...
        public function __construct()
        {
            ...
            add_shortcode('ehx_events', [$this, 'add_shortcode']);
        ...
        /**
         * Adds a shortcode for displaying a donation form for a specific event.
         *
         * The function retrieves the current event ID, retrieves the associated post and custom fields,
         * fetches all published 'ehx-event' posts, and generates a donation form with the necessary fields.
        ...
        public function add_shortcode()
        {
            ...
            [$post, $ehx_event] = self::get_field($event_id);

            $events = [];
            if ($post == null || ($post != null && $post->post_type != 'ehx-event')) {
                $args = [
                    'post_type' => 'ehx-event',
                ...
                foreach (get_posts($args) as $post) {
                    $events[$post->post_name] = $post->post_title;
                }
            }
            ...
            require EHX_DONATE_PLUGIN_DIR . 'views/shortcodes/event.php';
        ...
        private function handlePayment($total_amount, $browser_session)
        {
            try {
                ...
                $event = get_page_by_path(page_path: $this->request->input('event'), post_type: 'ehx-event'); 

                EHX_Donate_Helper::sessionSet('event', $event);
                ...
                $priceData = [
                    ...
                    'product_data' => [
                        ...
                        // 'description' => substr(strip_tags($event->post_content), 20),
                    ],
                ];
        ...
        private function paymentCallback($status, $txid, $browser_session): bool|string
        {
            ...
            $event = EHX_Donate_Helper::sessionGet('event');
            ...
            EHX_Donate_Helper::sessionForget('event');
        ...
        private static function get_field($form_id): array
        {
            ...
            $ehx_event = get_post_meta($post->ID, '_ehx_event', true) ?? [];
            return [$post, $ehx_event];
        }
    }
}